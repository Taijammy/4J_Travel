...
function TrackContent() {
  ...
  useEffect(() => {
    if (rideId) fetchActiveRide().finally(() => setPageReady(true));
  }, [rideId]);
  ...
  if (!pageReady) return <TrackSkeleton />;
  if (!ride) return (
    <div className="flex h-screen items-center justify-center bg-[#0f0f0f] flex-col gap-4">
      <p className="text-4xl">🚖</p>
      <p className="text-white font-semibold">Ride not found</p>
  ...
  return (
    <div className="h-screen bg-[#0f0f0f] flex flex-col">
      ...
      <div className="bg-[#0f0f0f] border-t border-[#1e1e1e] rounded-t-2xl overflow-y-auto" style={{ maxHeight:"58vh" }}>
        ...
        <div className="px-4 pb-6 pt-2 space-y-4">
          ...
          <DriverDistance pickup={ride.pickup} driverLocation={driverLoc} rideStatus={ride.status} />
          ...
          {driver && (
            <div className="flex items-center gap-3 bg-[#1a1a1a] border border-[#252525] rounded-xl p-3">
              ...
              <div className="text-right shrink-0 space-y-1">
                ...
                {driver.phone && <a href={`tel:${driver.phone}`} className="text-xs text-gray-500 hover:text-yellow-400 block">Call</a>}
              </div>
            </div>
          )}
          ...
          <p className="text-[11px] text-gray-600 font-mono text-center">Ride #{rideId?.slice(-8).toUpperCase()}</p>

          {isDone ? (
            <Button full size="lg" onClick={() => router.push("/dashboard")}>Back to Home</Button>
          ) : canCancel && (
            <Button full size="lg" variant="danger" onClick={() => cancelRide("Customer cancelled")}>Cancel Ride</Button>
          )}
        </div>
      </div>
    </div>
